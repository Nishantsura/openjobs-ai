(() => {
  ...
  const SMART_CLASS = 'openjobs-smart-apply-glassdoor';
  const FLOATING_ID = 'openjobs-smart-apply-glassdoor-floating';

  function findForm() {
    return selectors.findGlassdoorForm(document);
  }

  function findApplyCta() {
    const node = selectors.findGlassdoorApplyCta?.(document);
  ...
  function attachFormButton(form) {
    ...
    button.addEventListener('click', async () => {
      ...
      try {
        ...
        adapter.showToast(statusFromResult(result, 'Glassdoor'));
      } catch (_err) {
        adapter.showToast('Smart Apply failed on Glassdoor. Continue manually.', 'error');
      } finally {
  ...
  function attachFloatingButton() {
    ...
    button.addEventListener('click', async () => {
      ...
      try {
        ...
        if (!form) {
          adapter.showToast('No Glassdoor application form detected on this page.', 'error');
          return;
        }
        const result = await adapter.runFormFill(form);
        adapter.showToast(statusFromResult(result, 'Glassdoor'));
